...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const submenuToggles = document.querySelectorAll('.md3-nav__submenu-toggle, .md3-nav__trigger');
  const navToggle = document.querySelector('.md3-nav__mobile-toggle, .nav-toggle');
  ...
  function closeAllSubmenus() {
    ...
    document.querySelectorAll('.md3-nav__submenu-toggle, .md3-nav__trigger').forEach((t) => t.setAttribute('aria-expanded', 'false'));
  }
  ...
  document.addEventListener('click', (e) => {
    if (!e.target.closest('.md3-nav__links') && !e.target.closest('.md3-nav__submenu')) {
      closeAllSubmenus();
    }
  });
  ...
  document.querySelectorAll('.md3-mobile-bottomsheet-trigger').forEach((btn) => {
    btn.addEventListener('click', () => {
      const id = btn.getAttribute('aria-controls');
      const panel = id ? document.getElementById(id) : null;
      if (!panel) return;
  ...
  document.querySelectorAll('.md3-bottomsheet').forEach((sheet) => {
    ...
    mobileSheetButtons.forEach(btn => {
      btn.addEventListener('click', (e) => {
        ...
        const triggerId = btn.id || ('md3-bsheet-trigger-' + Math.random().toString(36).slice(2,8));
        btn.id = triggerId;
    ...
    document.querySelectorAll('.md3-bottomsheet').forEach(sheet => {
      const backdrop = sheet.querySelector('.md3-bottomsheet__backdrop');
      if (backdrop) backdrop.addEventListener('click', () => closeBottomSheet(sheet));
      const closeBtn = sheet.querySelector('.md3-bottomsheet__close');
      if (closeBtn) closeBtn.addEventListener('click', () => closeBottomSheet(sheet));
      // trap Escape
    ...
    const subdrawerTriggers = document.querySelectorAll('.md3-mobile-subdrawer-trigger');
    function openSubdrawer(drawer, trigger) {
      drawer.removeAttribute('hidden');
      drawer.setAttribute('aria-hidden', 'false');
      // store trigger id for focus return
      const triggerId = trigger.id || ('md3-subdrawer-trigger-' + Math.random().toString(36).slice(2,8));
    ...
    document.querySelectorAll('.md3-subdrawer').forEach(drawer => {
      ...
      if (backdrop) backdrop.addEventListener('click', () => closeSubdrawer(drawer));
      if (backBtn) backBtn.addEventListener('click', () => closeSubdrawer(drawer));
      drawer.addEventListener('keydown', (ev) => { if (ev.key === 'Escape') closeSubdrawer(drawer); });
    });

    // NEW: nested subpanel inside mobile menu (mkdocs-like)
    const mobilePanel = document.querySelector('.md3-mobile-menu__panel');
    const subpanelTriggers = document.querySelectorAll('.md3-mobile-subdrawer-trigger');
    subpanelTriggers.forEach(btn => {
      btn.addEventListener('click', (ev) => {
        ...
        const ensureMenuOpen = () => {
          ...
          const linkCount = Array.from(links).filter(l => l.matches('a, button')).length;
          if (linkCount <= 8) {
          ...
          // store trigger id for focus return
          const tid = btn.id || ('md3-subpanel-trigger-' + Math.random().toString(36).slice(2,8));
          btn.id = tid; panel.dataset.triggerId = tid;
          // set aria-expanded
          btn.setAttribute('aria-expanded','true');
          // focus first link inside
          const first = panel.querySelector('a, button, [tabindex]'); if (first) first.focus();
        };
    ...
    document.querySelectorAll('.md3-subpanel__back').forEach(back => {
      back.addEventListener('click', (ev) => {
        ...
        panel.setAttribute('hidden',''); panel.setAttribute('aria-hidden','true');
        mobilePanel.classList.remove('has-subpanel');
        // restore focus to trigger
        const tid = panel.dataset.triggerId; if (tid){ const t = document.getElementById(tid); if (t) { t.setAttribute('aria-expanded','false'); t.focus(); } }
    ...
    if (mobileClose && mobileMenuRoot && mobilePanel) {
      mobileClose.addEventListener('click', () => {
        mobilePanel.classList.remove('has-subpanel');
        document.querySelectorAll('.md3-mobile-subpanel').forEach(p => { p.setAttribute('hidden',''); p.setAttribute('aria-hidden','true'); });
        // remove any inline fallback styles added when panels were opened
        try {
          document.querySelectorAll('.md3-mobile-subpanel').forEach(p => { p.style.zIndex = ''; p.style.pointerEvents = ''; });
          const inner = mobilePanel.querySelector('.md3-mobile-menu__inner'); if (inner) inner.style.pointerEvents = '';
        } catch(e){}
      });
    }
    ...
    (function markActiveLinks(){
      try{
        ...
        document.querySelectorAll('.md3-nav__submenu-link, .md3-subdrawer__link').forEach(a=>{
          try{
      ...
      }catch(e){console.warn('active-link marker failed',e)}
    })();
});
