 ...
 * @description Optimierte Player-Logik mit Buchstabenmarkierung und Tooltip-Funktion.
 */
...
(function () {
  ...
  // Funktion: Liste zurücksetzen
  function resetTokenCollector() {
    collectedTokenIds = [];
    updateTokenCollectorDisplay();
  }

  // Funktion: In Zwischenablage kopieren
  function copyTokenListToClipboard() {
    const input = document.getElementById('tokenCollectorInput');
    navigator.clipboard.writeText(input.value)
      .then(() => console.log('Token-IDs kopiert:', input.value))
      .catch(err => console.error('Fehler beim Kopieren:', err));
  }

  // Events an Icons binden
  document.addEventListener('DOMContentLoaded', () => {
    document.getElementById('copyTokenList')?.addEventListener('click', copyTokenListToClipboard);
    document.getElementById('resetTokenList')?.addEventListener('click', resetTokenCollector);
  });
  ...
  function loadTranscription(transcriptionFile, visualAudioPlayer) {
    ...
    const speakerAltMapping = {
      "lib-pm": `<span class="tooltip-high">Modo: </span>libre<br>
                 ...
                 <span class="tooltip-high">Sexo: </span>masculino`,
      "lib-pf": `<span class="tooltip-high">Modo: </span>habla libre<br>
                 <span class="tooltip-high">Hablante: </span>profesional<br>
                 <span class="tooltip-high">Sexo: </span>femenino`,
