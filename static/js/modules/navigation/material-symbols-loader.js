...
function testMaterialSymbolsRendering() {
  ...
  setTimeout(() => {
    ...
    const isLoaded = fontFamily.includes('Material Symbols Rounded') && width > 0 && width < 30;
...
async function checkAndApplyFallback() {
  ...
  if (!isLoaded) {
    console.warn('[Material Symbols] Font did not load, applying fallback (hiding icons)');
...
function hideFailedIcons() {
  ...
  icons.forEach(icon => {
    const parent = icon.closest('.md3-navigation-drawer__item, .md3-icon-button');
