...
function ensureAccordionFor(section) {
  console.log('[Turbo Accordion] ensureAccordionFor called:', { section, currentSection });
  ...
  const openBtn = drawer.querySelector('.md3-navigation-drawer__trigger[aria-expanded="true"]');
  const openSection = openBtn ? openBtn.getAttribute('data-section') : null;
  
  console.log('[Turbo Accordion] Current open section:', openSection, '→ Target section:', section);
  ...
  if (section) {
    const btn = drawer.querySelector(`.md3-navigation-drawer__trigger[data-section="${section}"]`);
    if (!btn) {
    ...
    const panel = panelId ? document.getElementById(panelId) : null;
    if (!panel) {
      console.warn('[Turbo Accordion] Panel element not found for button:', btn);
      currentSection = section;
      return;
    }
...
function highlightNavigationFromURL(pathname) {
  ...
  drawer.querySelectorAll('a[aria-current], .md3-navigation-drawer__trigger').forEach(el => {
    el.removeAttribute('aria-current');
    el.classList.remove('md3-navigation-drawer__item--active', 'md3-navigation-drawer__subitem--active');
  });
...
function saveDrawerState() {
  const openSubmenu = document.querySelector('.md3-navigation-drawer__submenu[data-open]');
  if (openSubmenu) {
...
export function initTurboIntegration() {
  ...
  document.addEventListener('turbo:load', () => {
    console.log('[Turbo] turbo:load event fired, pathname:', window.location.pathname);
  ...
  console.log('[Turbo Integration] Initialized with diff-based accordion and hydration guard');
}
