...
export class TopAppBar {
  ...
  initLoginSheet() {
    ...
    const closeButtons = document.querySelectorAll('[data-action="close-login"]');
    ...
    if (backdrop) {
      backdrop.addEventListener('click', () => this.closeLoginSheet(loginSheet));
    }
  ...
  openLoginSheet(sheet) {
    ...
    // Focus first input
    const firstInput = sheet.querySelector('input[type="text"], input[name="username"]');
    if (firstInput) {
