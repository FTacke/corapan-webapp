...
document.addEventListener('DOMContentLoaded', () => {
  ...
  const updateHidden = () => {
    sensitiveHidden.value = sensitiveCheckbox && sensitiveCheckbox.checked ? '1' : '0';
    includeRegionalHidden.value = includeRegionalCheckbox && includeRegionalCheckbox.checked ? '1' : '0';
  };

  // Update before submission and on change
  if (sensitiveCheckbox) sensitiveCheckbox.addEventListener('change', updateHidden);
  if (includeRegionalCheckbox) includeRegionalCheckbox.addEventListener('change', updateHidden);
