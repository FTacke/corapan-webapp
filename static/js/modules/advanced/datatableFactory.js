  ...
  return {
    ...
    columnDefs: [
      ...
      { // left context
        targets: 1,
        data: 'context_left',
        render: function(data) { return `<span class="md3-corpus-context">${escapeHtml(data || '')}</span>`; },
      ...
      { // match
        targets: 2,
        data: 'text',
        render: function(data) { return `<span class="md3-corpus-keyword"><mark>${escapeHtml(data || '')}</mark></span>`; },
      ...
      { // audio
        targets: 4,
        data: null,
        render: function(data, type, row) { return renderAudioButtons(row); },
        width: '120px',
        orderable: false,
        className: 'md3-datatable__cell--audio center-align'
      },
      { targets: 5, data: 'country_code', render: function(data, type) { if (type === 'sort' || type === 'filter' || type === 'type') return data || ''; return escapeHtml((data || '-').toUpperCase()); }, width: '80px' },
      { targets: 6, data: 'speaker_type', render: function(data) { return escapeHtml(data || '-'); }, width: '80px' },
      { targets: 7, data: 'sex', render: function(data) { return escapeHtml(data || '-'); }, width: '80px' },
      { targets: 8, data: 'mode', render: function(data) { return escapeHtml(data || '-'); }, width: '80px' },
      { targets: 9, data: 'discourse', render: function(data) { return escapeHtml(data || '-'); }, width: '80px' },
      { targets: 10, data: 'token_id', render: function(data) { return escapeHtml(data || '-'); }, width: '100px' },
      { targets: 11, data: 'filename', render: function(data, type, row) { return renderFileLink(data, type, row); }, width: '80px', className: 'center-align' }
    ],
    language: {
