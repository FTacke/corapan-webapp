...
1. Setzt `CORAPAN_RUNTIME_ROOT` automatisch auf den aktiven Dev-Runtime-Root:
	- `<WorkspaceRoot>` mit `data/` und `media/` neben `app/`
...
- App: `C:\dev\corapan\app`
- Data: `C:\dev\corapan\data`
- Media: `C:\dev\corapan\media`

Wenn `data/` und `media/` neben `app/` existieren, wird dieser übergeordnete Ordner als Runtime-Root verwendet.
