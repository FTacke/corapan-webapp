...
# Resolve roots
# $PSScriptRoot is app\scripts\blacklab
...
Write-Host "  App Root:        $webappRoot" -ForegroundColor Cyan
Write-Host "  Data Root:       $dataRoot" -ForegroundColor Cyan
Write-Host "  Config Root:     $configRoot (versioned app config)" -ForegroundColor Cyan
...
if (Test-Path $repoLocalDataRoot) {
    Write-Host "  FEHLER: Repo-local app\\data exists." -ForegroundColor Red
    Write-Host "    Dev BlackLab must mount the sibling workspace root data\\blacklab tree, not app\\data." -ForegroundColor Yellow
    exit 1
}
...
if (-not (Test-Path $indexPath)) {
    ...
    Write-Host "    .\app\scripts\blacklab\build_blacklab_index.ps1" -ForegroundColor Gray
    exit 1
}
