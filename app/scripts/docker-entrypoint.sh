...
if [ -z "$AUTH_DATABASE_URL" ]; then
    echo "ERROR: AUTH_DATABASE_URL is required for auth/core startup."
    exit 1
fi

if [[ "$AUTH_DATABASE_URL" != postgresql* ]]; then
    echo "ERROR: AUTH_DATABASE_URL must point to PostgreSQL for auth/core startup."
    exit 1
fi

echo "Waiting for PostgreSQL database to be ready..."

# Extract host and port from AUTH_DATABASE_URL
