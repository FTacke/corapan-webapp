...
RULES = {
    ...
    "MD3-HERO-001": (
        "ERROR",
        "Hero missing canonical structure (md3-hero--card md3-hero__container)",
    ),
    "MD3-HERO-002": (
        "ERROR",
        "Hero icon using <span> directly (use <div class='md3-hero__icon'><span>)",
    ),
    ...
    "MD3-ALERT-005": (
        "ERROR",
        "Alert icon class order wrong (material-symbols-rounded must come first)",
    ),
    "MD3-FIELD-ERROR-001": (
        "WARNING",
        "Error text not directly after md3-outlined-textfield block",
    ),
    "MD3-FIELD-ERROR-002": (
        "WARNING",
        'Input with aria-invalid="true" without linked md3-field-error',
    ),
    ...
    "MD3-TEXTFIELD-001": (
        "WARNING",
        "Non-standard textfield (use md3-outlined-textfield)",
    ),
...
PATTERNS = {
    ...
    "spacing_utilities": re.compile(
        r"\b(?:m|mt|mb|ml|mr|mx|my|p|pt|pb|pl|pr|px|py)-\d+\b"
    ),
    "mx_auto": re.compile(r"\b(mx-auto|m-auto)\b"),
    "inline_spacing": re.compile(
        r'style\s*=\s*"[^"]*(?:margin|padding)\s*:\s*\d+px[^"]*"', re.I
    ),
...
NON_CRITICAL_PATHS = {
    ...
    "docs/",
    "tests/e2e/",
}  # Design previews, docs, test outputs
...
def lint_dialog_structure(content: str, rel_path: str) -> List[LintIssue]:
    ...
    dialog_pattern = re.compile(
        r'<(?:dialog|div)[^>]*class\s*=\s*"[^"]*\bmd3-dialog(?:\s|"|$)[^"]*"[^>]*>',
        re.I,
    )
        ...
        if "md3-dialog__surface" not in dialog_block:
            issues.append(
                LintIssue(
                    ...
                    rel_path,
                    line,
                    "Dialog missing md3-dialog__surface",
                    tag[:60],
                )
            )
        ...
        if "md3-dialog__content" not in dialog_block:
            issues.append(
                LintIssue(
                    ...
                    rel_path,
                    line,
                    "Dialog missing md3-dialog__content",
                    tag[:60],
                )
            )
        ...
        if "md3-dialog__actions" not in dialog_block:
            issues.append(
                LintIssue(
                    ...
                    rel_path,
                    line,
                    "Dialog missing md3-dialog__actions",
                    tag[:60],
                )
            )

        if (
            "md3-dialog__title" not in dialog_block
            and "md3-title-large" not in dialog_block
        ):
            issues.append(
                LintIssue(
                    "MD3-DIALOG-004",
                    ...
                    rel_path,
                    line,
                    "Dialog missing md3-dialog__title",
                    tag[:60],
                )
            )
        ...
        if "aria-labelledby" not in tag and "aria-label" not in tag:
            issues.append(
                LintIssue(
                    ...
                    rel_path,
                    line,
                    "Dialog missing aria-labelledby or aria-label",
                    tag[:60],
                )
            )
...
def lint_card_structure(content: str, rel_path: str) -> List[LintIssue]:
    ...
    card_pattern = re.compile(
        r'<(?:article|div|section)[^>]*class\s*=\s*"[^"]*\bmd3-card\b[^"]*"[^>]*>', re.I
    )
        ...
        if not has_content:
            issues.append(
                LintIssue(
                    ...
                    rel_path,
                    line,
                    "Card missing md3-card__content",
                    tag[:60],
                )
            )
...
def lint_hero_structure(content: str, rel_path: str) -> List[LintIssue]:
    ...
    hero_pattern = re.compile(
        r'<(?:div|section)[^>]*class\s*=\s*"([^"]*\bmd3-hero\b[^"]*)"[^>]*>', re.I
    )
    ...
    for match in hero_pattern.finditer(content):
        ...
        bad_icon_patterns = [
            ...
            re.compile(
                r'<span[^>]*class\s*=\s*"[^"]*material-symbols[^"]*md3-hero__icon[^"]*"[^>]*>',
                re.I,
            ),
        ]
        ...
        eyebrow_span = re.search(
            r'<span[^>]*class\s*=\s*"[^"]*md3-hero__eyebrow[^"]*"[^>]*>',
            hero_block,
            re.I,
        )
        if eyebrow_span:
        ...
        title_match = re.search(
            r'<h1[^>]*class\s*=\s*"([^"]*md3-hero__title[^"]*)"[^>]*>', hero_block, re.I
        )
        if title_match:
...
def lint_legacy_patterns(content: str, rel_path: str) -> List[LintIssue]:
    ...
    for match in re.finditer(r'class\s*=\s*"([^"]*)"', content):
        ...
        if card_pattern.search(cls):
            ...
            if (
                "md3-card" in cls
                or "md3-hero--card" in cls
                or "md3-metric-card" in cls
                or "md3-admin-card" in cls
            ):
                continue
            line = get_line_number(content, match.start())
            issues.append(
                LintIssue(
                    ...
                    rel_path,
                    line,
                    "Legacy card class usage (use md3-card)",
                    match.group(0)[:60],
                )
            )
...
def lint_alert_patterns(content: str, rel_path: str) -> List[LintIssue]:
    ...
    error_alert_pattern = re.compile(
        r'<[^>]*class\s*=\s*"[^"]*\bmd3-alert--error\b[^"]*"[^>]*>', re.I
    )
    ...
    icon_wrong_order_pattern = re.compile(
        r'class\s*=\s*"md3-alert__icon\s+material-symbols-rounded"', re.I
    )
    ...
    aria_invalid_pattern = re.compile(
        r'<input[^>]*aria-invalid\s*=\s*["\']true["\'][^>]*>', re.I
    )
    for match in aria_invalid_pattern.finditer(content):
        ...
        describedby_match = re.search(
            r'aria-describedby\s*=\s*["\']([^"\']+)["\']', tag
        )
        if describedby_match:
            ...
            for ref_id in ids:
                ...
                error_pattern = re.compile(
                    rf'id\s*=\s*["\']?{re.escape(ref_id)}["\']?[^>]*class\s*=\s*"[^"]*md3-field-error',
                    re.I,
                )
                error_pattern_alt = re.compile(
                    rf'class\s*=\s*"[^"]*md3-field-error[^"]*"[^>]*id\s*=\s*["\']?{re.escape(ref_id)}',
                    re.I,
                )
...
def inventory_alert_patterns(content: str, rel_path: str) -> List[LintIssue]:
    ...
    support_pattern = re.compile(
        r'<[^>]*class\s*=\s*"[^"]*md3-field-support[^"]*"[^>]*>', re.I
    )
    ...
    error_pattern = re.compile(
        r'<[^>]*class\s*=\s*"[^"]*md3-field-error[^"]*"[^>]*>', re.I
    )
...
def inventory_form_fields(content: str, rel_path: str) -> List[LintIssue]:
    ...
    for match in input_pattern.finditer(content):
        ...
        if (
            ...
            in content[max(0, match.start() - 200) : match.start()]
        ):
...
def scan_file(path: Path, root: Path, options: dict) -> List[LintIssue]:
    ...
    any(rel_path.startswith(z.rstrip("/")) for z in SPECIAL_CSS_ZONE)
    ...
    if is_html:
        ...
        if is_datatables:
            for issue in file_issues:
                ...
                if issue.severity == "ERROR" and not issue.rule_id.startswith(
                    "MD3-ALERT"
                ):
                    issue.severity = "INFO"
...
def scan_directory(
    root: Path, focus_paths: Optional[List[str]] = None, options: Optional[dict] = None
) -> List[LintIssue]:
...
def generate_markdown_report(issues: List[LintIssue], output_path: Path):
    ...
    with output_path.open("w", encoding="utf8") as f:
        ...
        if errors:
            ...
            for issue in errors:
                f.write(
                    f"| `{issue.file}` | {issue.line} | {issue.rule_id} | {issue.message} |\n"
                )
            f.write("\n")
        ...
        if info:
            f.write("## Info\n\n")
            f.write(
                "<details><summary>Click to expand ({} items)</summary>\n\n".format(
                    len(info)
                )
            )
            f.write("| File | Line | Rule | Message |\n")
...
def main():
    ...
    parser.add_argument(
        "--focus", nargs="+", help="Focus scan on specific paths (relative to root)"
    )
    parser.add_argument(
        "--out", default="reports/md3-lint-auto.md", help="Markdown report output path"
    )
