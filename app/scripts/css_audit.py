...
class CSSAuditor:
    ...
    def audit_important_usage(self, file_path: Path, content: str) -> None:
        ...
        if not any(
            str(file_path).endswith(allowed) for allowed in self.IMPORTANT_ALLOWED
        ):
        ...
        for line_num, line in enumerate(lines, 1):
            if (
                "!important" in line
                and "NEEDS_IMPORTANT"
                not in content[max(0, content.find(line) - 500) : content.find(line)]
            ):
                self.violations.append(
                    Violation(
                        file_path=str(file_path.relative_to(self.repo_root)),
    ...
    def audit_font_family(self, file_path: Path, content: str) -> None:
        ...
        pattern = re.compile(
            r'font-family:\s*[^;]*"Courier New"[^;]*"Consolas"', re.IGNORECASE
        )
    ...
    def audit_inline_styles_js(self, file_path: Path, content: str) -> None:
        ...
        patterns = [
            re.compile(
                r"\.style\.(background|color|border|padding|margin)\s*=", re.IGNORECASE
            ),
            re.compile(r'\.setAttribute\(\s*["\']style["\']', re.IGNORECASE),
            re.compile(
                r'\.setProperty\(\s*["\']--[a-z-]+["\']', re.IGNORECASE
