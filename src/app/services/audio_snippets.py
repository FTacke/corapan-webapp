...
def find_split_file(filename: str, start: float, end: float) -> Optional[Tuple[Path, str]]:
    """
    ...
    for suffix, (split_start, split_end) in SPLIT_TIMES.items():
        if split_start <= start and end <= split_end:
            ...
            stem = Path(filename).stem  # "2022-01-18_VEN_RCR" from "2022-01-18_VEN_RCR.mp3"
...
def build_snippet(filename: str, start: float, end: float, token_id: str | None = None, snippet_type: str | None = None) -> Path:
    """
