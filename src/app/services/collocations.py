...
def get_collocations(cql: str, window_size: int = 5, subcorpus_filter: str = "") -> Dict:
    """
    ...
    try:
        ...
        response = client.get(f"{BLS_BASE_URL}/corpora/corapan/hits", params=colloc_params)
        ...
        return {
            "left": [{"word": word, "n": count, "p": count / total_left if total_left else 0} for word, count in left_words.most_common(20)],
            "right": [{"word": word, "n": count, "p": count / total_right if total_right else 0} for word, count in right_words.most_common(20)],
            "total_hits": data.get("summary", {}).get("resultsStats", {}).get("hits", 0),
        }

    except httpx.HTTPStatusError as e:
        logger.error(f"BlackLab error fetching collocations: {e.response.status_code} - {e.response.text}")
        return {"error": "bls_error", "message": f"BlackLab error: {e.response.status_code}"}
