...
def validate_cql_pattern(cql: str) -> str:
    ...
    if paren_count != 0:
        raise CQLValidationError(f"Unmatched opening parenthesis '(' (count: {paren_count})")
    if bracket_count != 0:
        raise CQLValidationError(f"Unmatched opening bracket '[' (count: {bracket_count})")
    ...
    for field, values in filters.items():
        ...
        for value in values:
            ...
            if re.search(r'[;&|`]', value):
                logger.warning(f"Suspicious characters in filter value: {field}={value}")
