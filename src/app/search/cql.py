...
def build_token_cql(
    token: str, mode: str, sensitive: bool, pos: Optional[str] = None
) -> str:
    """
...
def build_speaker_code_constraint(filters: Dict) -> str:
    ...
    matching_codes = get_speaker_codes_for_filters(
        speaker_types=speaker_types, sexes=sexes, modes=modes, discourses=discourses
    )

    if not matching_codes:
        # No matching codes → impossible filter (will return 0 results)
        logger.warning(
            f"No speaker_codes match filters: speaker_type={speaker_types}, sex={sexes}, mode={modes}, discourse={discourses}"
        )
...
def build_filters(params: Dict) -> Dict[str, any]:
    ...
    include_regional = params.get("include_regional") in (
        "1",
        "true",
        "on",
        True,
    ...
    filters["country_code"] = [c.strip().upper() for c in raw_codes if c and c.strip()]

    # Speaker types (multi-select) - no lowercasing
    speaker_types = (
        params.getlist("speaker_type")
        if hasattr(params, "getlist")
        else (
            params.get("speaker_type", [])
            if isinstance(params.get("speaker_type"), list)
            else ([params.get("speaker_type")] if params.get("speaker_type") else [])
        )
    )
    ...
    sexes = (
        ...
        if hasattr(params, "getlist")
            ...
            params.get("sex", [])
            if isinstance(params.get("sex"), list)
            else ([params.get("sex")] if params.get("sex") else [])
        )
    )
    ...
    modes = (
        params.getlist("speech_mode")
        if hasattr(params, "getlist")
        else (
            params.get("speech_mode", [])
            if isinstance(params.get("speech_mode"), list)
            else ([params.get("speech_mode")] if params.get("speech_mode") else [])
        )
    )
    ...
    if not modes:
        ...
        if mode_param and mode_param not in [
            "forma",
            "lemma",
            "cql",
            "forma_exacta",
            ...
            modes = (
                ...
                if hasattr(params, "getlist")
                    ...
                    else (mode_param if isinstance(mode_param, list) else [])
                )
            )
    ...
    discourses = (
        params.getlist("discourse")
        if hasattr(params, "getlist")
        else (
            params.get("discourse", [])
            if isinstance(params.get("discourse"), list)
            else ([params.get("discourse")] if params.get("discourse") else [])
        )
    )
...
def resolve_countries_for_include_regional(
    countries: Optional[list], include_regional: bool
) -> list:
    """
