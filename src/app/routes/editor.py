...
def add_bookmark():
    ...
    try:
        ...
        existing = next((b for b in transcript_data["bookmarks"] if b.get("segment_index") == segment_index), None)
        if existing:
            return {"success": False, "message": "Bookmark already exists for this segment"}, 400
...
def remove_bookmark():
    ...
    try:
        ...
        if "bookmarks" in transcript_data:
            transcript_data["bookmarks"] = [
                b for b in transcript_data["bookmarks"] 
        ...
        return {"success": False, "message": f"Failed to remove bookmark: {str(e)}"}, 500
...
def undo_change():
    ...
    try:
        ...
        state = _reconstruct_from_diffs(original_backup_path, diffs_file, until_index=undo_index-1)
...
def _cleanup_old_backups(backup_dir: Path, stem: str, max_backups: int = 10) -> None:
    ...
    try:
        ...
        pattern = f"{stem}_[0-9]{{8}}_[0-9]{{6}}.json"
        backups = sorted(
            [f for f in backup_dir.glob(f"{stem}_*.json") 
        ...
        print(f"[Backup] Kept {min(len(backups), max_backups)} timestamped backups + 1 original")
...
def _reconstruct_from_diffs(original_path: Path, diffs_file: Path, until_index: int = None) -> dict:
    """
            ...
            with open(diffs_file, "r", encoding="utf-8") as f:
                for line in f:
                    ...
                    for change in changes:
                        ...
                        if (seg_idx is not None and word_idx is not None and 
                            seg_idx < len(state.get("segments", [])) and
                            word_idx < len(state["segments"][seg_idx].get("words", []))):
                            
                            state["segments"][seg_idx]["words"][word_idx]["value"] = new_val
