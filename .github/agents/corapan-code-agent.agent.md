...
It must treat the root repository model explicitly:
- `CORAPAN/.github` controls local agent behavior and active versioned CI/deploy behavior
- `app/` is the versioned app/deploy implementation subtree
- `CORAPAN/` is the workspace and repository root
...
- if root-level workspace files and repo-level deploy files both exist, classify which one governs the current task before changing either
...
The agent must treat `CORAPAN/.github/workflows` as the active deploy/CI truth for the root repository.
...
- the local workspace root is `CORAPAN/`
- `app/` is the versioned app/deploy implementation
- local runtime data under `CORAPAN/data/...` outranks `app/data` for local dev analysis unless runtime disproves that
- local BlackLab index state is mutable operational state and must not be rebuilt while BlackLab is serving it
- a real hits query is stronger evidence than BlackLab root readiness
...
Interpret these local dev paths from the `CORAPAN/` workspace root.
...
- blacklab-operational-safety for BlackLab hits failures, index rebuild/start guardrails, active-index validation, or index-corruption triage
...
- the task is ambiguous between local agent governance and versioned workflow behavior under `CORAPAN/.github`
- a valid BlackLab hits failure could be either app logic or active-index corruption and live evidence is unavailable
