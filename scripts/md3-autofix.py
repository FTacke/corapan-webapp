...
def scan_and_fix(root: Path, scope: Optional[str] = None, apply: bool = False) -> List[Fix]:
...
def main():
    parser = argparse.ArgumentParser(
        description='MD3 Conservative Auto-Fix Script'
    )
    parser.add_argument('--root', default=str(ROOT),
                        help='Root directory')
    parser.add_argument('--scope',
                        help='Scope to specific path (relative to root)')
    parser.add_argument('--apply', action='store_true',
                        help='Actually apply fixes (default: dry-run)')
    parser.add_argument('--dry-run', action='store_true',
                        help='Show what would be fixed without changing files')
    parser.add_argument('--report', default='docs/md3-template/md3_autofix_report.json',
