...
from src.app.services.audio_snippets import find_split_file, build_snippet, SPLIT_TIMES
from src.app.services.media_store import MP3_SPLIT_DIR, MP3_TEMP_DIR

BL_PROXY_BASE = "http://127.0.0.1:8000/bls"

LISTVALUES = "tokid,start_ms,end_ms,filename,file_id,word,lemma,country,country_code,radio"
...
def main(tokens):
    for tok in tokens:
        ...
        for i, hit in enumerate(hits[:3]):
            ...
            for k in ['token_id','filename','start_ms','end_ms','context_start','context_end','text','country_code']:
            ...
            split_res = None
            if filename:
                split_res = find_split_file(f"{filename}.mp3" if not filename.endswith('.mp3') else filename, start_s, end_s)
            if split_res:
            ...
            try:
                ...
                snippet = build_snippet(f"{filename}.mp3" if not filename.endswith('.mp3') else filename, start_s, end_s, token_id=tok, snippet_type='pal')
