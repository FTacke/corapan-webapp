...
def flatten_doc(infile: Path) -> Dict[str, Any]:
    ...
    out = {k: doc.get(k, '') for k in (
        'file_id','filename','date','country_code','country_scope','country_parent_code',
        'country_region_code','city','radio','audio_path'
    )}
    ...
    for segment in doc.get('segments', []):
        ...
        speaker_type = seg_speaker.get('speaker_type') or segment.get('speaker_type', '')
        speaker_sex = seg_speaker.get('speaker_sex') or segment.get('speaker_sex', '')
        speaker_mode = seg_speaker.get('speaker_mode') or segment.get('speaker_mode', '')
        speaker_discourse = seg_speaker.get('speaker_discourse') or segment.get('speaker_discourse', '')
...
def main() -> int:
    ...
    parser.add_argument('--limit', type=int, default=0, help='optional limit of files to process')
