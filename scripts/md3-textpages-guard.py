...
def find_textpage_templates():
    ...
    for p in sorted(PAGES_DIR.glob('*.html')):
        ...
        if any(k in basename for k in EXCLUDE_KEYWORDS):
            ...
            continue
        is_named = basename in DEFAULT_TEXTPAGE_NAMES or basename.startswith('proyecto_')
...
def extract_main_block(text):
    m = re.search(r"<main[^>]*class=['\"][^'\"]*md3-text-page[^'\"]*['\"][^>]*>(.*?)</main>", text, re.S | re.I)
...
def check_file(path, text):
    ...
    if 'class="md3-page__header"' not in text and "class='md3-page__header'" not in text:
    ...
    section_matches = re.findall(r"<section[^>]*class=['\"]([^'\"]*)['\"][^>]*>", main, re.I | re.S) if main else []
    if main and not any('md3-text-section' in cls for cls in section_matches):
        issues['structure'].append('no section with class="md3-text-section" found in main')
    ...
    if main:
        ...
        for h2_match in h2s:
            ...
            if 'md3-title-large' not in classes or 'md3-section-title' not in classes:
                issues['headings'].append(f'H2 missing required classes (found: "{classes}")')
        ...
        for h3_match in h3s:
            ...
            cls_m = re.search(r"class=['\"]([^'\"]*)['\"]", tag)
            classes = cls_m.group(1) if cls_m else ''
            if 'md3-title-medium' not in classes or 'md3-subsection-title' not in classes:
                issues['headings'].append(f'H3 missing required classes (found: "{classes}")')
    ...
    for m in re.finditer(r"<section[^>]*class=['\"][^'\"]*md3-text-section--divider[^'\"]*['\"][^>]*>", text, re.I | re.S):
    ...
    if divider_positions:
        ...
        if len(divider_positions) != len(h2_positions):
            issues['dividers'].append(f'divider count ({len(divider_positions)}) != h2 count ({len(h2_positions)})')
        ...
        for kind, pos in divider_positions:
            ...
            if next_h2 is None:
                ...
                if not re.match(r'^[\s\n\r\t\{\%\#]*$', between):
                    issues['dividers'].append(f'divider at pos {pos} not immediately followed by H2 (contains other content)')
...
def main():
    ...
    if overall['STRUCTURE_FAIL'] or overall['HEADINGS_FAIL']:
        print('md3-textpages-guard: STRUCTURE/HEADING failures found (see report):', REPORT_PATH)
        raise SystemExit(2)
    elif overall['DIVIDERS_WARN']:
        print('md3-textpages-guard: Warnings found (dividers) — see report:', REPORT_PATH)
        raise SystemExit(0)
    else:
