...
def main():
    ...
    parser.add_argument('--apply', action='store_true', help='Apply changes in-place (creates .bak files)')
    parser.add_argument('--preview-only', action='store_true', help='Print only preview and do not apply')
    ...
    print('\nSummary:')
    print(f' {len(changed_files)} files contain matches, approx {total_changes} total replacements.')
    if args.apply:
