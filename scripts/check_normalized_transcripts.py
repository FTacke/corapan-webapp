    ...
    for field in REQUIRED_HEADER_FIELDS:
        if field not in data:
            errors.append(ValidationError(
                json_path,
                "MISSING_FIELD",
                f"Pflichtfeld '{field}' fehlt im Header"
            ))
...
def validate_segment(segment: dict, seg_idx: int, json_path: Path) -> List[ValidationError]:
    ...
    if "speaker_code" not in segment:
        errors.append(ValidationError(
            json_path,
            "MISSING_FIELD",
            f"Segment {seg_idx}: 'speaker_code' fehlt"
        ))
...
def validate_token(word: dict, seg_idx: int, word_idx: int, json_path: Path) -> List[ValidationError]:
...
def validate_json(json_path: Path) -> List[ValidationError]:
    ...
    if not isinstance(segments, list):
        errors.append(ValidationError(
            json_path,
            "MISSING_OBJECT",
            "'segments' ist keine Liste oder fehlt"
        ))
        return errors
    ...
    for seg_idx, segment in enumerate(segments):
        if not isinstance(segment, dict):
            errors.append(ValidationError(
                json_path,
                "INVALID_TYPE",
                f"Segment {seg_idx} ist kein Objekt"
            ))
            continue
