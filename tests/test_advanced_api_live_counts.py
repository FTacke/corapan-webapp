...
def bls_available():
    try:
        with httpx.Client(timeout=TIMEOUT) as c:
            r = c.get(f"{BASE_BLS}/corpora/corapan/hits", params={"first": 0, "number": 0})
...
def test_advanced_api_returns_counts_against_real_bls():
    ...
    with httpx.Client(timeout=TIMEOUT) as client:
        ...
        bls_resp = client.get(f"{BASE_BLS}/corpora/corapan/hits", params=bls_params, headers={"Accept": "application/json"})
        ...
        flask_params = {"draw": 1, "start": 0, "length": 10, "q": "casa", "mode": "lemma"}
        flask_resp = client.get(f"{BASE_FLASK}/search/advanced/data", params=flask_params)
        ...
        if bls_hits <= 0:
            pytest.skip("BlackLab returned zero hits for this test token; skipping live assertion")
        assert records_total > 0 or records_filtered > 0, "Flask advanced API returned zero results when BL does have hits"
        # Optionally, don't assert equality; but we assert that Flask doesn't return more than BL
        assert records_total <= bls_hits or records_total == 0, "Flask reported more hits than BL (unexpected)" 
