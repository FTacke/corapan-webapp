...
def test_datatables_json_structure(client):
    ...
    logger.info(f"✓ JSON structure valid: {len(data['data'])} results, {data['recordsTotal']} total")
...
def test_kwic_context_present(client):
    ...
    for idx, row in enumerate(data['data']):
        ...
        assert isinstance(row['context_left'], str), f"Row {idx}: 'context_left' should be string"
        assert isinstance(row['context_right'], str), f"Row {idx}: 'context_right' should be string"
...
def test_metadata_fields_populated(client):
    ...
    for idx, row in enumerate(data['data']):
        ...
        assert row.get('speaker_type') is not None, f"Row {idx}: speaker_type should not be None"
        ...
        assert row.get('discourse') is not None, f"Row {idx}: discourse should not be None"
...
def test_include_regional_logic(client):
    ...
    response_national = client.get(f'/search/advanced/data?{urlencode(params_national)}')
    ...
    if len(data_national['data']) > 0:
        ...
        for row in data_national['data']:
            country = row.get('country_code', '').upper()
            assert country not in regional_codes, f"Found regional code {country} when include_regional=0"
        
        logger.info(f"✓ include_regional=0: No regional codes found in {len(data_national['data'])} results")
    ...
    response_regional = client.get(f'/search/advanced/data?{urlencode(params_regional)}')
    ...
    logger.info(f"✓ include_regional logic validated: national={data_national['recordsTotal']}, regional={data_regional['recordsTotal']}")
...
def test_case_preservation_in_tokens(client):
    ...
    assert has_uppercase, "Expected at least some uppercase letters in token text (original case should be preserved)"
...
def test_example_sentence_forma_mujer_insensitive(client):
    ...
    # Should find results
    assert data['recordsTotal'] > 0, "Expected to find results for 'mujer' (insensitive)"
    
    logger.info(f"✓ Found {data['recordsTotal']} results for 'mujer' (case-insensitive)")
...
def test_audio_metadata_present(client):
    ...
    for idx, row in enumerate(data['data']):
        ...
        if row.get('filename'):
            assert isinstance(row['start_ms'], (int, float)), f"Row {idx}: start_ms should be numeric"
...
def test_pagination_parameters(client):
    ...
    logger.info(f"✓ Pagination validated: page 1 and page 2 return different results")
