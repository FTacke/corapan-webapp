...
def test_password_forgot_and_reset(client):
    ...
    rc = client.post('/auth/reset-password/confirm', json={'resetToken': raw, 'newPassword': 'newpass123'})
    assert rc.status_code == 200

    # login with new password
    l = client.post('/auth/login', json={'username': 'e2e_reset', 'password': 'newpass123'})
    assert l.status_code in (200, 303, 204)


def test_admin_create_user_via_api_and_verify_ui(client):
    from src.app.auth import services

    admin = create_user('e2e_admin', role='admin')
    # login as admin and set cookies
    resp = client.post('/auth/login', json={'username': 'e2e_admin', 'password': 'password123'})
    for s in resp.headers.getlist('Set-Cookie'):
        try:
            ...
            pass
    token = services.create_access_token_for_user(services.find_user_by_username_or_email('e2e_admin'))
    client.set_cookie('access_token_cookie', token, path='/')

    new_user_resp = client.post('/admin/users', json={'username': 'invited_e2e', 'email': 'invite@e2e.test'})
